...
@property (nonatomic) double startMoveTime;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.startMoveTime = CURRENT_TIME;
...
- (void)drawStep {
    if ((CURRENT_TIME - self.startMoveTime) > 0.5f) {
        [self moveCharacterPosition];
        self.startMoveTime = CURRENT_TIME;
    }
...
- (void)moveCharacterPosition {
    if ([self.boardView isThereTargetSlot]) {
        [self.boardView moveCharacterTo:[self.boardView currentTarget]];
    }
}
